...
if [ -z "$JDK" -a -s "$HOME/.IdeaIC2018.3/config/idea.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.IdeaIC2018.3/config/idea.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.IdeaIC2018.3/config/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.IdeaIC2018.3/config/idea$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=IdeaIC2018.3 \
